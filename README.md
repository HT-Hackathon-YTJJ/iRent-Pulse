...
**L0 決定「這張算不算過」，不決定「什麼時候按」，也不決定「要不要拍」。**
舊版一達標就自己拍，那個交易比看起來差：自動化喜歡的那一幀很少是人正要拍的那一幀，
它在人還在移動時就響了，而且拿走了流程裡唯一一個由使用者確認「這張對了」的時刻。

所以只有按鈕會拍。而且**按下去一定曝光一張**：綠燈就直接進格子，黃燈或灰燈就把
畫面**定格在剛拍下的那張照片**上，跳出「判定未達標」，讓人對著看得到的那張照片
決定要「重拍這張」還是「仍要送出」——按「仍要送出」送的就是定格的那張，不是回答
問題當下鏡頭對著的東西。

這件事比聽起來重要。L0 會判錯：車子明明好好框在輪廓線裡，偵測器在昏暗車場或深色
牆前面就是漏掉，而那正是使用者會去按「仍要送出」的情況。如果照片是那時候才拍，
使用者花一分鐘對好的那一幀就被丟掉，換成手機放下來時拍到的柏油路——歸檔進了一個
使用者以為裝著自家保險桿的格子。

快門是**被守門，不是被鎖死**：在昏暗停車場怎麼樣都轉不成綠燈的人必須能完成還車，
代價是一張普通的照片，而拒絕的代價是整趟還車。同一格站超過 8 秒門檻自動放寬
20%，24 秒後直接開放，而且**按「重拍這張」不會把這個時鐘歸零**——同一格重拍是
同一次嘗試的延續，歸零等於每按一次重拍就把人踢回最嚴格的門檻，那個放寬就永遠等
不到它本來要救的那個人。換格子才重新計時。

這段守則寫在 `_onShutter` 的註解裡；如果哪天要拿掉那個逃生門，要一起改的就是那段
註解。
...
的 toast：通知送達時 App 通常已經在背景或已經關掉。點下去會開
`lib/screens/order_detail_screen.dart`（Figma 1010:6493）。

以前還有一個 `ReturnNoticeBanner`，把 Figma 那張鎖定畫面照著畫成 App 內橫幅，
跟系統通知一起送，好讓投影機、靜音的模擬器、Web 版都看得到結果抵達。問題是那是
iOS 樣式的通知，畫在 Android 手機的 App 裡面，本來要讓人覺得「系統在講話」的那
一刻，反而看起來像個 bug。現在只剩通知欄——它才是真的產品行為，App 關掉也送得
到，而且長得跟使用者自己手機一樣。沒有權限就不會收到，訂單明細裡照樣查得到。
