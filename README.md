...
還車拍照的 AI 檢測**預設打已部署的後端**，不需要在本機起任何東西：

<https://irent-pulse-api.fly.dev>

所以裝了 APK 的同仁不必跟我們連同一個 Wi-Fi。要改打本機的 `api/run.sh`：

```bash
api/run.sh
flutter run --dart-define=API_BASE_URL=http://10.0.2.2:8000   # Android 模擬器
```

部署（`api/fly.toml`）：

```bash
tool/deploy_fly.sh --with-key
```
...
| **車牌比對** | ML Kit 端上文字辨識，裁 bbox 後每 650ms 讀一次，比對租借車牌 | `l0/plate.dart`、`l0/plate_reader.dart` |
...
四個車身角落多一項檢查：畫面裡的車牌，是不是租約上那一台。不是的話輪廓線變紅、
badge 顯示「車輛不符」、提示列寫「這不是你租的車」。

**用 ML Kit 的通用文字辨識，不是專門的車牌模型。** 專用 ALPR 是兩段式的（先框
出車牌、再認字元），要找到授權可用的權重、轉成 TFLite、再對台灣車牌調兩個模型
——而我們沒有那個資料集。ML Kit 是一次呼叫、免費、已經針對真實場景拍到的文字調
過，而且**「車牌在哪」這一段我們已經有答案了**：COCO 偵測器每一幀都給出車體
bbox，裁下去就好，專用 ALPR 第一段網路要回答的問題我們用一次裁切就解決。

代價是它會把車上所有文字都讀回來（車款徽章、貼紙、驗車標籤），分辨哪個是車牌
變成 `l0/plate.dart` 的工作——那比字元辨識容易得多。

**這個檢查允許往哪個方向錯。** 對著正確的車說「這不是你租的車」，比對著錯誤的車
保持沉默糟糕得多：使用者站在對的車前面被趕走，而且無從辯解。所以：

* 讀不到、讀不清、單一幀不一致，一律當作**沒有意見**，畫面跟這個功能不存在時一樣。
* 字形折疊（O↔0、I↔1、S↔5、B↔8）兩邊都套用，再加一個字元的編輯距離容忍。
  15 公尺外的雨天車牌會少一個字，另一台車不會只差一個字。
* 要**連續三次**讀成別台車才轉紅（約兩秒）。
* 一旦確認是這台車就記住兩分鐘——使用者接著要繞車拍另外三個角，那裡根本沒有
  車牌可讀，忘記的話會在第二個角開始跟他吵架。
* 旁邊那台車的車牌常常比自己的清楚，所以**已確認的比對不會被路過的讀數推翻**。

紅框**不鎖快門**。`AimState.wrongCar` 只是「不是 locked」，走的是既有的那條路：
按下快門照樣曝光、畫面凍結、跳出面板給「重拍這張」與「仍要送出」。辨識可能是錯的，
而那正是使用者最需要覆寫權的時刻；車牌判定會寫進 `l0` 區塊送給 L1，真的拍錯車還有
後面那層負責。

規則本身在 `test/plate_test.dart` 有 18 個測試——OCR 那段是 method channel，
在開發機上沒辦法驗，但「會不會把人趕走」的判斷全部在純 Dart 這一側。

### 拍攝順序不再是單行道

底下那條 7 格的膠捲可以點。點哪一格就切到哪一格拍，包含**已經拍過的**——重拍就是
這樣做的。拍完當前這格會自動跳到**下一個**還沒拍的格子（往前走、走完繞回頭），
不是跳回膠捲最前面那個漏掉的。

原本它是一個使用者無法介入的佇列：對某一張不滿意，唯一的辦法是把七張拍完、等分析
跑完、然後祈禱它剛好標出你早就知道的那一張。往前走而不是「第一個沒拍的」，是因為
使用者正繞著車走，膠捲的順序就是那圈路線——剛重拍完左前，把他送回駕駛座門邊的
加油卡是多走一趟，而下一個角落是他現在正面對的那個。

判定未達標的面板打開時膠捲是不能點的：那個面板是在問一張特定照片的去留，走開會
讓那張照片懸在半空。
...
不是每一格都畫輪廓線，因為不是每一格都有可以對齊的東西：

* **四個車角** — 車體渲染圖，依 [AimState] 上色。有一台車在那裡，而且要的是一個
  可重複的角度，所以把真實的鈑件對到畫出來的鈑件是有意義的。
* **加油卡/停車卡** — 卡套的框，**用程式畫的**（`_CardPouchPainter`），不是圖檔。
  這一格要的是一個**版面**：iRent 的遮陽板卡套是一片品牌面板加左右兩個插袋，
  使用者要拍到的是這三塊、而且左右不能顛倒。畫某一個卡套的渲染圖會多講很多事
  （絨布顏色、磨損、裡面插著什麼卡）而且大部分是錯的。三個圓角矩形加兩個標籤，
  向量畫出來在任何尺寸都是銳利的，也不用進 bundle。
* **前座／後座** — 什麼都不畫。手機離椅背一個手掌寬，每個人的坐姿都把它擺在不同
  地方，畫出來的輪廓沒有人對得上，而「你對不上」就是它唯一講得出口的事。
  底下 strip 的 slot 縮圖仍然有示意圖：講**要拍哪裡**是有用的，
  講邊緣要落在哪裡不是。

所以 `assets/images/return/guide/` 只有四個車角的圖檔；
`tool/gen_slot_assets.py` 的 `GUIDED` 就是這件事，
`test/capture_slots_test.dart` 會確認另外三格**沒有**被打包進去。

三個車內的 slot 另外拿到 `aimSlack = 0.35`：門檻的預設值是照著「白天、手臂距離、
一整台車」調的，車內是屋頂底下的深色近拍，常常還有自己的影子蓋在上面，
那組模糊下限與曝光上限在裡面根本到不了。**一個永遠綠不了的 slot，
會讓人一直重試到放棄還車**，那是 L0 唯一不被允許的失敗模式。
...
**2026-09-08 之後**：七張母檔都是 1440×1440，放在 `tool/slot_paint/`（無損 WebP，
約 2 MB；刻意不放 `assets/` 底下，才不會被打包進 App）。匡線是 1.4 倍放大而不是
28 倍，strip 縮圖是 288×288 的**縮小**——4 倍於 72pt 的格子，對得上 3.125 的螢幕。

在那之前上游全部都是 72×72——設計 repo 是，Figma 板子裡的圖片填充也是
（node 843:900 原生匯出只有 55×37），所以畫質只能靠**怎麼放大**贏：
...
這些機制對已經夠銳利的母檔沒有壞處，所以留著：1.4 倍之下重新切邊對乾淨的邊緣是
沒有作用的操作，unsharp 的半徑跟著倍率縮放，等於自己關掉。`--fetch` 仍然可以回去
抓設計 repo 的 72px 原圖，走的就是上面那條路。
...
* **探得到** → 情境 ⓪「真實 AI 檢測」：相機跑 L0，每拍完一張立刻送 L1，
  ...
  L2／L3。琥珀那格點下去會凍在那張照片上，把 L1 的原話蓋上去問「重拍還是保留」。
  分析頁只有兩條進度條；問題全部列在它後面那一頁，一顆按鈕重拍全部。
